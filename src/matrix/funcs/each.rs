...
impl<K: Float + SubAssign + AddAssign> Matrix<K> {
    ...
    pub fn add_each_col(&self, v: &Vector<K>) -> Matrix<K> {
        assert_eq!(
            self.rows,
            v.size(),
            "Row sizes must be equal to add the vector to the each column of the matrix!"
        );
        let mut new_data = self.data.clone();
        for r in 0..self.rows {
            let val = v.data[r];
    ...
    pub fn add_each_row(&self, v: &Vector<K>) -> Matrix<K> {
        assert_eq!(
            self.cols,
            v.size(),
            "Columns sizes must be equal to add the vector to the each row of the matrix!"
        );
        let mut new_data = self.data.clone();
        for r in 0..self.rows {
            for c in 0..self.cols {
